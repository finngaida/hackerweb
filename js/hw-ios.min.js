...
var c=h.hostname.replace(/^www\./,""),d=h.pathname.replace(/^\//,"").split("/")[0],e=d.length,f=c.length<=25&&e>3&&15>=e&&/^[^0-9][^.]+$/.test(d)?"/"+d:"";return b=c+f,i[a]=b,b},k=b.querySelector("#view-home .scroll"),l=k.querySelector("section"),m=!1,n=function(){if(!(!1 in window))if(console.log(Notification.permission),"default"===Notification.permission)Notification.requestPermission(function(){n()});else if("granted"===Notification.permission){var a=new Notification("New article on HN",{body:"Title of the article here",
tag:"some unique string"});a.onclick=function(){this.close()},a.onclose=function(){console.log("Notification closed")}}else if("denied"===Notification.permission)return};f.news={options:{disclosure:!0},markupStory:function(a){return/^item/i.test(a.url)?a.url="#/item/"+a.id:(a.external=!0,a.domain=j(a.url)),f.news.options.disclosure?("link"==a.type&&(a.detail_disclosure=!0),/^#\//.test(a.url)&&(a.detail_disclosure=!1,a.disclosure=!0,a.domain=null)):a.id&&(a.url="#/item/"+a.id),a.i_point=1==a.points?"point":"points",
a.i_comment=1==a.comments_count?"comment":"comments",g("post",a)},markupStories:function(a,b){var c="";b||(b=1);var d=f.news.markupStory;return a.forEach(function(a){a.i=b++,c+=d(a)}),c},updateStory:function(a){if(a&&a.id){var b,d=a.id,e=a.data,g="hacker-news",h=amplify.store(g);if(h)for(var i=0,j=h.length;j>i;i++){var k=h[i];if(d==k.id){b=k;break}}if(!b&&(g="hacker-news2",h=amplify.store(g)))for(var i=0,j=h.length;j>i;i++){var k=h[i];if(d==k.id){b=k;break}}if(b){var l=!1;if(["title","url","time_ago","comments_count","points"].forEach(function(a){
var c=e[a];b[a]!=c&&(b[a]=c,l=!0)}),l){n(),amplify.store(g,h);var m=c("story-"+d);m&&(b.selected=!!m.querySelector("a[href].selected"),b.i=m.dataset?m.dataset.index:m.getAttribute("data-index"),m.insertAdjacentHTML("afterend",f.news.markupStory(b)),m.parentNode.removeChild(m))}}}},render:function(a){if(!m){a||(a={});var b=amplify.store("hacker-news-cached"),d=g("stories-load"),e=function(a){n();var b=a.slice(),c='<ul class="tableview tableview-links" id="hwlist">'+f.news.markupStories(b)+(amplify.store("hacker-news2")?'<li><a class="more-link">More&hellip;<span class="loader"><i class="icon-loading"></i></span></a></li>':"")+"</ul>";
...
c("hwlist").insertAdjacentHTML("beforeend",g)}},400)}}};var o=c("view-comments"),p=o.querySelector("header h1"),q=o.querySelector("section");f.comments={currentID:null,render:function(a){if(a){var c=amplify.store.sessionStorage("hacker-item-"+a);if(f.comments.currentID!=a||!c){f.comments.currentID=a;var d=function(a,c){var d=e(a),h=g("post-comments");if(d.has_post=!!d.title,!d.has_post)return f.setTitle(),p.innerHTML="",q.innerHTML=h.render(d),f.pub("adjustCommentsSection"),void f.pub("onRenderComments");
...
}d.short_hn_url="news.ycombinator.com/item?id="+c,d.hn_url="//"+d.short_hn_url,f.setTitle(d.title),p.innerHTML=d.title;var m=h.render(d,{comments_list:i}),n=b.createElement("div");n.innerHTML=m;for(var o=n.querySelectorAll("a"),r=0,s=o.length;s>r;r++){var t=o[r];t.target="_blank"}var u=d.user;if(u)for(var v=n.querySelectorAll(".user"),r=0,s=v.length;s>r;r++){var w=v[r];w.textContent.trim()==u&&(w.classList.add("op"),w.title="Original Poster")}if(m.length>2e4)for(var x=n.querySelectorAll(".comments>ul>li>ul"),y=g("comments-toggle"),z=0,s=x.length;s>z;z++){
var A=x[z],B=A.querySelectorAll(".metadata").length;A.style.display="none",B&&A.insertAdjacentHTML("beforebegin",y.render({comments_count:B,i_reply:1==B?"reply":"replies"}))}for(;q.hasChildNodes();)q.removeChild(q.childNodes[0]);for(;n.hasChildNodes();)q.appendChild(n.childNodes[0]);delete n,f.pub("onRenderComments")};if(c)q.scrollTop=0,d(c,a);else{var h=amplify.store("hacker-news");if(h)for(var i=0,k=h.length;k>i;i++){var l=h[i];if(a==l.id){c=l;break}}if(!c){var h=amplify.store("hacker-news2");if(h)for(var i=0,k=h.length;k>i;i++){
